...
Desired behavior (parity with embedded/CIRCUITPY build):
- A1 OPEN (disconnected): Normal mode. USB mass‑storage writable from host
  and CircuitPython. Use this to drag/drop files from Windows/macOS.
- A1 CLOSED (to GND): Provisioning mode. USB mass‑storage is write‑protected
  (host cannot modify files) so the device firmware can safely write Wi‑Fi
  and provisioning files itself during setup.
"""
...
# Switch OPEN (pull-up HIGH) -> Normal Mode (host writable)
...
if not provision_switch.value:  # Switch closed (A1 connected to GND)
    ...
    # Device can write; host cannot (disable USB drive exposure)
    storage.remount("/", readonly=False)
    try:
