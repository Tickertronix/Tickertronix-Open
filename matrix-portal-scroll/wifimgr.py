...
def load_device_config():
    """Load device configuration (hub settings)."""
    try:
...
def get_connection():
    ...
    # Check if device has hub config
    device_config = load_device_config()
    if not device_config or 'hub_base_url' not in device_config:
        print("[WIFI] Device not provisioned for hub, starting setup...")
        start_provisioning()
        while True:
            time.sleep(1)
